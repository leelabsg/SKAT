...
SKATBinary_spa<-function (G, obj, Cutoff,method="All")
{
    ...
    if (length(maf_temp) > 0 & length(maf_temp) < length(MAF)) {
        weight[maf_temp] = SKAT:::Beta_Weight(MAF[maf_temp], c(1, 25))
        weight[-maf_temp] = SKAT:::Beta_Weight(MAF[-maf_temp], c(0.5,
            0.5))
        flag = 1
    }    else {
        if (length(maf_temp) == 0) {
            weight = SKAT:::Beta_Weight(MAF, c(0.5, 0.5))
            flag = 2
        }
        if (length(maf_temp) == length(MAF)) {
            weight = SKAT:::Beta_Weight(MAF, c(1, 25))
            flag = 3
        }
    }
