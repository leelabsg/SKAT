...
SKATBinary_spa<-function (G, obj, Cutoff=2,method="SKATO",r.corr=NULL){
    ...
    if (length(maf_temp) > 0 & length(maf_temp) < length(MAF)) {
       	weight[maf_temp] = SKAT:::Beta_Weight(MAF[maf_temp], weights.beta.rare)
       	weight[-maf_temp] = SKAT:::Beta_Weight(MAF[-maf_temp], weights.beta.common)
       	flag = 1
    }    else {
        if (length(maf_temp) == 0) {
            weight = SKAT:::Beta_Weight(MAF,weights.beta.common)
            flag = 2
        }
        if (length(maf_temp) == length(MAF)) {
            weight = SKAT:::Beta_Weight(MAF,  weights.beta.rare)
            flag = 3
        }
    }
    ...
    if (flag == 2) {
        list_myfun$rare_n = 0
        list_myfun$common_n = length(MAF)
        list_myfun$rare_mac = 0
        list_myfun$common_mac = sum(G)
    }
    if (flag == 1) {
        list_myfun$rare_n = length(maf_temp)
        list_myfun$common_n = length(MAF) - length(maf_temp)
        list_myfun$rare_mac = sum(G[, maf_temp])
        list_myfun$common_mac = sum(G[, -maf_temp])
    }
    if (flag == 3) {
        list_myfun$rare_n = length(MAF)
        list_myfun$common_n = 0
        list_myfun$rare_mac = sum(G)
        list_myfun$common_mac = 0
    }
...
, estimate_MAF=1){
	...
	is.run<-FALSE
        ...
        re<-SKATBinary_spa(G=Z,obj=obj.res,weights.beta.rare= weights.beta.rare, weights.beta.common= weights.beta.common, weights = weights, method=method, r.corr=r.corr, CommonRare_Cutoff= CommonRare_Cutoff)
